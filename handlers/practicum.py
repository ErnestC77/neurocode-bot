...
from services.catalog import PRACTICUM, PRODUCT_PRICE_RUB, get_available_products
...
async def practicum_details(callback: CallbackQuery) -> None:
    ...
    await callback.message.answer(TEXTS["M6.2"], reply_markup=practicum_buy_kb(available))
...
async def practicum_buy(callback: CallbackQuery, config: Config) -> None:
    ...
    amount = PRODUCT_PRICE_RUB[PRACTICUM]
