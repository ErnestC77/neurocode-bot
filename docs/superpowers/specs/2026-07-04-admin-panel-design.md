...
DB-настройки "Доп. владелец".

**Важно:** `owner_chat_id` / `get_effective_owner_chat_id` используются не
только для авторизации — это ещё и получатель уведомлений о лидах/оплатах
(`exports/notifier.py`) и алертов о смене прав бота в канале
(`handlers/admin.py::bot_membership_changed`). Эта роль "куда слать
уведомления" остаётся как есть, без изменений — трогаем только проверку
доступа.
...
- При первом обращении, если таблица пуста, автоматически засеивается
  ...
  миграции.
- `is_authorized_admin(tg_id, config)` переписывается на: состоит ли tg_id в
  `admins`, ИЛИ `tg_id == config.owner_chat_id` (env-бутстрап остаётся как
  запасной вариант, как и раньше). DB-настройка "Доп. владелец" и
  `get_effective_owner_chat_id` не трогаются — они по-прежнему определяют
  только адресата уведомлений.
